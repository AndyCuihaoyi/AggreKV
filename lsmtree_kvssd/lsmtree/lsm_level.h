...
{
    // only cache one kind of data(meta or kv)
    keyset *sets;
    //	uint8_t *bitset;
    uint8_t iscached; // for compaction(partial_leveling)
    /*
       when the cache is used for header reading in compaction,
       but It can be evicted by new inserted cache after compaction, so we should check it
       0->not cached
...
{
    keyset sets[PAGESIZE / KEYSETSIZE]; // 8KB/8,一个page可以存多少个key-ppa对（keyset）
    char *nocpy_table;
    value_set *origin;
} htable_t;
...
typedef struct run
{
    ...
    htable *cpt_data; // temporary pointer
    void *run_data;
    char *level_caching_data; // cached metadata
...
{
    /*level operation*/
    level *(*init)(int size, int idx, bool istier);
    void (*release)(level *);
