...
#include "include/x86_64-linux-gnu/sys/mman.h"
...
uint32_t demand_create(algorithm *palgo, lower_info *li) {
    ...
    D_ENV(palgo)->max_cache_entry =
        (D_ENV(palgo)->num_grain / EPP) + ((D_ENV(palgo)->num_grain % EPP != 0)
                                               ? 1
                                               : 0); // total number of tpages

    D_ENV(palgo)->num_block = _NOS;
    D_ENV(palgo)->p_p_b = _PPS;
    D_ENV(palgo)->num_tblock =
        (D_ENV(palgo)->max_cache_entry / D_ENV(palgo)->p_p_b) +
        (D_ENV(palgo)->max_cache_entry % D_ENV(palgo)->p_p_b ? 1 : 0);
    D_ENV(palgo)->num_tblock =
        (D_ENV(palgo)->num_tblock < 2) ? 2 : D_ENV(palgo)->num_tblock;
    // num_tblock      = ((num_block / EPP) + ((num_block % EPP != 0) ? 1 : 0))
    // * 4;
    D_ENV(palgo)->num_tpage = D_ENV(palgo)->num_tblock * D_ENV(palgo)->p_p_b;
    D_ENV(palgo)->num_dblock =
        D_ENV(palgo)->num_block - D_ENV(palgo)->num_tblock - 2;
    ...
    /* Cache control & Init */
    D_ENV(palgo)->nr_pages_optimal_caching =
        (D_ENV(palgo)->num_page * 4 / PAGESIZE);
    num_max_cache = nr_pages_optimal_caching / 20;
    ...
    real_keys = (KEYT *)malloc(
        sizeof(KEYT) * D_ENV(palgo)->pd_cache->env.nr_valid_tpages * EPP);
    if (-1 == mlock(real_keys, sizeof(KEYT) *
                                   D_ENV(palgo)->pd_cache->env.nr_valid_tpages *
                                   EPP)) {
        ftl_err("real_keys mlock() failed!\n");
        abort();
    }

    memset(D_ENV(palgo)->r_hash_collision_cnt, 0,
           sizeof(D_ENV(palgo)->r_hash_collision_cnt));
    memset(D_ENV(palgo)->w_hash_collision_cnt, 0,
           sizeof(D_ENV(palgo)->w_hash_collision_cnt));
...
lpa_t get_lpa(demand_cache *pd_cache, KEYT key, void *_h_params) {
    ...
    uint32_t lpa = PROBING_FUNC(h_params->hash, h_params->cnt) %
                   pd_cache->env.nr_valid_tentries;
    return lpa;
}
...
read_retry:
    ...
    if (IS_INFLIGHT(req->params)) {
        struct inflight_params *i_params =
            (struct inflight_params *)req->params;
...
data_read:
    ...
    rc = read_actual_dpage(pte.ppa,
                           req); // after async read, should check full key.
    if (rc == UINT32_MAX) {
        goto read_ret;
    }
