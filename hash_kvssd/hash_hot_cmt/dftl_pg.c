...
{
#ifdef DATA_SEGREGATION
    /* DS 模式：dftl_bm_init 已准备好 MAX_GC_STREAM 个 d_reserve（is_rsv=true），
     * 各流 active 也已就位。这里不再分配，避免重复占用。 */
    d_reserve = NULL;
    d_active = NULL;
#else
...
ppa_t dp_alloc(block_mgr_t *bm, lpa_t lpa)
{
    ...
    if (stream->active_sblk == NULL) {
      ftl_err("[dp_alloc] stream=%d active_sblk is NULL, lpa=%lu\n", idx, (unsigned long)lpa);
      abort();
    }
    ...
    if (!d_active || bm->check_full(bm, d_active))
    {
        if (bm->isgc_needed(bm, DATA_S))
        {
            ...
            ftl_log("DATA GC - (valid/total: %d/%d)\n", nr_valid_pages, _PPS);
#endif
        }
        else
        {
...
#ifdef PRINT_GC_STATUS
            ftl_log("TRANS GC - (valid/total: %d/%d)\n", nr_valid_pages, _PPS);
#endif
        }
        else
        {
...
/* 改为 thread-local：7 个 worker 线程并发触发 GC 时，共享全局变量会 race condition
 * （gcb_node_arr 中的指针指向 gc_bucket 内部，malloc/free 跨线程会段错） */
__thread struct gc_bucket *gc_bucket;
__thread struct gc_bucket_node *gcb_node_arr[_PPS * GRAIN_PER_PAGE];
static int _do_bulk_write_valid_items(block_mgr_t *bm,
                                      gc_table_struct **bulk_table,
                                      int nr_read_pages, page_t type, bm_superblock_t *target_seg)
{
...
#ifdef DATA_SEGREGATION
    /* DS 模式 GC bulk write：完全照搬单流非 DS 模式连续段写法。
     *  当 ppa 不连续（切流或切 batch）时 flush 旧段。
     *  target_seg->stream_idx 表达"这个 victim 块原本属于哪个流"，valid grain
     *  回写到该流的 active sblk（保持 locality）。 */
    int32_t stream_idx = target_seg->stream_idx;
    if (stream_idx < 0 || stream_idx >= MAX_GC_STREAM) {
        /* 异常兜底：target 不属于任何已知流（理论上不会发生），用触发 GC 的流 */
        stream_idx = 0;
    }
    ...
    for (int i = 0; i < nr_valid_items; i++)
    {
        ...
        {
            ppa = gc_dp_alloc(bm, stream_idx);
            stream->active_sblk_offset = stream->active_sblk->wp_offt;
            stream->active_ppa = ppa;
            stream->grain_remain = GRAIN_PER_PAGE;
            if (sppa == -2)
            {
                sppa = ppa;
            }
            else if (ppa != last_ppa + 1)
            {
                /* 不连续：flush 旧段 [sppa, last_ppa + 1) */
                uint64_t lat = __demand.li->write(
                    sppa, (uint64_t)(last_ppa - sppa + 1) * PAGESIZE, 0);
                if (lat == (uint64_t)-1 || lat == (uint64_t)-2)
                {
                    ftl_err("[gc bulk write] sppa=%llu length=%llu lat=%lu\n",
                            (unsigned long long)sppa,
                            (unsigned long long)(last_ppa - sppa + 1),
                            (unsigned long)lat);
                    abort();
                }
                sppa = ppa;
        ...
        for (int j = 0; j < len; j++)
        {
            bm->validate_grain(bm, node->ppa + j);
        }
        bm_oob_t new_oob = {
            .is_tpage = false,
            .lpa = node->lpa,
    ...
    /* flush 最后一段 */
    if (sppa != -2 && last_ppa != -2)
    {
        uint64_t lat = __demand.li->write(
            sppa, (uint64_t)(last_ppa - sppa + 1) * PAGESIZE, 0);
        if (lat == (uint64_t)-1 || lat == (uint64_t)-2)
        {
            ftl_err("[gc bulk write last] sppa=%llu length=%llu lat=%lu\n",
                    (unsigned long long)sppa,
                    (unsigned long long)(last_ppa - sppa + 1),
                    (unsigned long)lat);
            abort();
    ...
    ftl_log("dGC: [valid grains: %d -> packed grains: %d], reclaimed: %llu\n",
            nr_valid_grains, copied_pages * GRAIN_PER_PAGE,
            (unsigned long long)(_PPS * GRAIN_PER_PAGE - copied_pages * GRAIN_PER_PAGE));
    /* 注意：gcb_node_arr 存的是 gc_bucket 内部地址，bulk_mapping_update 必须在 free 之前。
     * 这里不在本函数 free gc_bucket，交给 dpage_gc_dvalue 末尾统一 free（但需要
     * dpage_gc_dvalue 在 bulk_mapping_update 之后再 free，否则 gcb_node_arr 悬空）。 */
                                   ...
                                   page_t type)
{
    ...
    for (int i = 0; i < nr_valid_grains; i++)
    {
        ...
        {
            ((demand_env *)__demand.env)->num_gc_mapping_hit++;
    ...
    ((demand_env *)__demand.env)->gc_cmt_total += cmt_cnt;
...
{
    __atomic_add_fetch(&gc_inflight, 1, __ATOMIC_RELAXED);
    ...
    if (unlikely(target_seg == NULL))
    {
        ftl_err("dpage_gc_dvalue: no GC target (stream=%d)\n", stream_idx);
        abort();
    }
    ...
    target_seg->gc_cnt++;
#ifdef DATA_SEGREGATION
    /* DS 模式（DATA 专属路径）：trim 完 target_seg 状态 reset
     *   - clear owned/stream_idx（trim_segment 内部已做） */
    target_seg->is_owned = false;
    target_seg->stream_idx = -1;
    /* rsv 池水位维护（仿单流 dftl_trim_segment 的 rsv 水位语义，
     *   但下沉到 DATA 专属路径，避免影响 MAP） */
    bm_part_ext_t *data_pt = &bm->env->part[DATA_S];
    if (data_pt->rsv_sblk_cnt < MAX_GC_STREAM) {
      /* 水位未满，trim 出的 sblk 进 d_reserve 池（GC 备用） */
      target_seg->is_rsv = true;
      data_pt->rsv_sblk_cnt++;
    } else {
      /* 水位已满，放回 free 池（与单流 sblk 复用语义一致） */
      target_seg->is_rsv = false;
      data_pt->free_sblk_cnt++;
    }
    ftl_assert(!target_seg->is_owned);
    ftl_assert(target_seg->stream_idx == -1);
#endif
    ...
    /* gc_bucket/gcb_node_arr 是 thread-local（多 worker 并发 GC 安全），
     * 释放本线程的 gc_bucket 并设 NULL */
    free(gc_bucket);
    gc_bucket = NULL;

    __atomic_sub_fetch(&gc_inflight, 1, __ATOMIC_RELAXED);
    return nr_read_pages;
}
...
int tpage_gc(block_mgr_t *bm)
{
    ...
    /* trim blocks on the gsegemnt */
    bm->trim_segment(bm, target_seg, __demand.li);
    /* 磨损均衡统计：本次 GC 完成，累加 target_seg 的 gc_cnt */
    target_seg->gc_cnt++;
...
#ifdef DATA_SEGREGATION
    /* 跳过 stream 的 in-flight 写入区：[flush_ppa, flush_ppa + flush_page)
     * —— wb path 已经 mark valid 但还没真正 ssd 写入的范围。 */
    for (int i = 0; i < MAX_GC_STREAM; i++)
    {
        bm_stream_manager_t *s = &bm->env->stream[i];
        if (real_ppa >= s->flush_ppa && real_ppa < s->flush_ppa + s->flush_page)
        {
            return 0;
        }
    }
