...
{
    d_reserve = bm->get_active_superblock(bm, DATA, true);
    d_active = NULL;

    t_reserve = bm->get_active_superblock(bm, MAP, true);
    t_active = NULL;
...
ppa_t dp_alloc(block_mgr_t *bm)
{
    ...
    if (!d_active || bm->check_full(bm, d_active))
    {
        ...
        {
            d_active = bm->get_active_superblock(bm, DATA, false);
        }
    }
...
ppa_t tp_alloc(block_mgr_t *bm)
{
    ...
    if (!t_active || bm->check_full(bm, t_active))
    {
        ...
        {
            t_active = bm->get_active_superblock(bm, MAP, false);
        }
    }
...
int dpage_gc_dvalue(block_mgr_t *bm)
{
    ...
    bm_superblock_t *target_seg = bm->get_gc_target(bm, DATA);
    ...
    d_active = d_reserve;
    d_reserve = bm->change_reserve(bm, DATA, tobe_reserve);
...
int tpage_gc(block_mgr_t *bm)
{
    ...
    bm_superblock_t *target_seg = bm->get_gc_target(bm, MAP);
    ...
    bm_superblock_t *tobe_reserve = target_seg;
    t_active = t_reserve;
    t_reserve = bm->change_reserve(bm, MAP, tobe_reserve);
