...
KEYT *real_keys;
static size_t real_keys_bytes;
static int real_keys_locked;
...
uint32_t demand_create(algorithm *palgo, lower_info *li)
{
    ...
    D_ENV(palgo)->num_gc_mapping_hit = 0;   
    D_ENV(palgo)->num_gc_flash_write = 0;
    D_ENV(palgo)->gc_cmt_total=0;
    ...
    real_keys_bytes = sizeof(KEYT) * D_ENV(palgo)->pd_cache->env.nr_valid_tpages * EPP;
    real_keys = (KEYT *)malloc(real_keys_bytes);
    if (!real_keys)
    {
        ftl_err("real_keys malloc() failed\n");
        abort();
    }
    real_keys_locked = (ftl_try_mlock(real_keys, real_keys_bytes, "real_keys") == 0);
...
void demand_destroy(algorithm *palgo, lower_info *li)
{
    ...
    palgo->finish_q = NULL;
    if (real_keys && real_keys_locked)
        munlock(real_keys, real_keys_bytes);
    free(real_keys);
    real_keys = NULL;
    real_keys_bytes = 0;
    pbm->destroy(pbm);
...
lpa_t get_lpa(demand_cache *pd_cache, KEYT key, void *_h_params)
{
    ...
    errno = 0;
    char *endptr;
    char key_buf[MAXKEYSIZE + 1] = {0};
    size_t key_len = (key.len > MAXKEYSIZE) ? MAXKEYSIZE : key.len;
    memcpy(key_buf, key.key, key_len);
    unsigned long val = strtoul(key_buf, &endptr, 10);

    // 安全检查（避免无效字符串）
    if (errno == 0 && endptr != key.key && val <= UINT32_MAX)
    {
        lpa = (uint32_t)val; // 此时lpa = 1，符合预期
    }
    else
    {
        lpa = 0; // 转换失败的默认值
...
#ifdef ADAPTIVE_MEM
        adaptive_ghost_cold_on_miss(pd_cache, pd_cache->member.cmt[D_IDX],
				    req->type == DATAW);
#endif
    cache_load:
        // rc = pd_cache->wait_if_flying(pd_cache,lpa, req, NULL);
        // if (rc)
        // {
        //     goto read_ret;
        // }
...
read_ret:
    if (req->type == PREFILL_DATAR && rc != UINT32_MAX)
    {
        return lpa;
    }
    return rc;
