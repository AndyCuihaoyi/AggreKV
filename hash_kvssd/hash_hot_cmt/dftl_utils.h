...
#define ftl_warn(fmt, ...)                                                                              \
    ...
    {                                                                                                   \
        uint64_t time_ns = clock_get_ns();                                                              \
        fprintf(stdout, "[%.6lf] [WARN] " fmt, (double)(time_ns - timer_start_ns) / 1e9, ##__VA_ARGS__); \
    } while (0)
...
static inline void warn_notfound(char *f, int l)
{
    ...
    ftl_warn("Read target data not found, at %s:%d\n", f, l);
#endif
}
...
/* mlock 失败时打一次 WARN 并继续（避免 memlock 上限导致 abort） */
