#include "../dftl_block/dftl_utils.h"
#include "ssd.h"
#include "lower.h"
#include <pthread.h>
#include <stdint.h>
...
struct ppa pgidx2ppa(struct ssd *ssd, uint32_t pgidx)
{
  ...
  struct ppa ppa = {
      ...
      .g.blk = (pgidx / spp->nchs / spp->luns_per_ch / spp->pls_per_lun) %
               spp->blks_per_pl,
      .g.pg = (pgidx / spp->nchs / spp->luns_per_ch / spp->pls_per_lun /
               spp->blks_per_pl) %
              spp->pgs_per_blk,
  };
...
void ssd_stats_init(lower_info *li)
{
  ...
  li->stats->nr_nand_rd_lun =
      g_malloc0(sizeof(uint64_t) * ssd_lower.sp.tt_luns);
  li->stats->nr_nand_wr_lun =
      g_malloc0(sizeof(uint64_t) * ssd_lower.sp.tt_luns);
  li->stats->nr_nand_er_lun =
      g_malloc0(sizeof(uint64_t) * ssd_lower.sp.tt_luns);
}
...
static void ssd_init_params(struct ssdparams *spp)
{
  spp->secsz = 512;        // 512
  spp->secs_per_pg = 8;    // 8
  spp->pgs_per_blk = 2048; // 128MB SBLK
  spp->blks_per_pl = 2356; /* 2048 * 1.15, very large OP */
  spp->pls_per_lun = 1;    // 1
  spp->luns_per_ch = 8;    // 8
  spp->nchs = 4;           // 4
  ...
  ftl_log("ssd created! size: %lu MB\n",
          (uint64_t)spp->secsz * spp->secs_per_pg * spp->pgs_per_blk *
              spp->blks_per_pl * spp->pls_per_lun * spp->luns_per_ch *
              spp->nchs / 1024 / 1024);
...
void ssd_destroy(lower_info *li)
{
  ...
  g_free(ssd->ch);
  ssd->ch = NULL;
  g_free(ssd_li.stats->nr_nand_rd_lun);
  g_free(ssd_li.stats->nr_nand_wr_lun);
  g_free(ssd_li.stats->nr_nand_er_lun);
  ssd_li.stats->nr_nand_rd_lun = NULL;
  ssd_li.stats->nr_nand_wr_lun = NULL;
  ssd_li.stats->nr_nand_er_lun = NULL;
}
...
static uint64_t ssd_advance_status(struct ssd *ssd, struct ppa *ppa, int ncmd,
                                   uint64_t stime)
{
  int c = ncmd;
...
uint64_t ssd_write_ppa(uint32_t pgidx, uint64_t size, uint64_t stime)
{
  ...
  for (; tmp_pgidx < pgidx + size / PAGESIZE; ++tmp_pgidx)
  {
    ...
    uint32_t target_offset_in_line =
        tmp_pgidx - line_idx * ssd_lower.sp.pgs_per_line;
    ...
    ssd_li.stats
        ->nr_nand_wr_lun[ppa.g.ch * ssd_lower.sp.luns_per_ch + ppa.g.lun]++;
    uint64_t etime =
        stime + ssd_advance_status(&ssd_lower, &ppa, NAND_WRITE, stime);
...
uint64_t ssd_read_ppa(uint32_t pgidx, uint64_t size, uint64_t stime)
{
  ...
  for (; tmp_pgidx < pgidx + size / PAGESIZE; ++tmp_pgidx)
  {
    ...
    struct ppa ppa = pgidx2ppa(&ssd_lower, tmp_pgidx);
    ssd_li.stats
        ->nr_nand_rd_lun[ppa.g.ch * ssd_lower.sp.luns_per_ch + ppa.g.lun]++;
    uint64_t etime =
        stime + ssd_advance_status(&ssd_lower, &ppa, NAND_READ, stime);
    max_etime = etime > max_etime ? etime : max_etime;
  }
  ftl_assert(max_etime >= stime);
...
uint64_t ssd_trim_block(uint32_t pgidx)
{
  ...
  for (int i = 0; i < ssd_lower.sp.tt_luns; ++i)
  {
    ...
    ssd_li.stats
        ->nr_nand_er_lun[ppa.g.ch * ssd_lower.sp.luns_per_ch + ppa.g.lun]++;
  ...
  memset(ssd_lower.ppa_state + pgidx, 0,
         sizeof(bool) * ssd_lower.sp.pgs_per_line);
