...
| **Evidence of verification & validation**: each sub-driver's expected output path is documented, and `test.sh` prints a clear pass/fail marker | Satisfied | `hash_kvssd/hash_kvssd_results/logs/<EXP_TAG>_<variant>.log` (E1–E9, suffixed with `_AggreKV` or `_RHIK`); `lsmtree_kvssd/lsmtree_test_results/summary.txt`; `block_ssd/blktrace_test_results/test_*.log` and `block_ssd/blktrace_summary/summary_<timestamp>.txt`. The `hash_kvssd` sub-driver does not write an aggregate summary file under Functional-badge review: per-experiment numbers are read from the `[SUMMARY]` marker inside each log (`grep '\\[SUMMARY\\]' logs/*.log`) (full list in `README.md` §"Outputs and How to Interpret Them"); success marker is `bash test.sh` exiting 0 with `[TEST OK] all three sub-drivers finished` |
