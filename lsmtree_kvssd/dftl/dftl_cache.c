...
static void cache_env_init(struct cache_env *env) {
    ...
    // env->nr_valid_tpages = (_NOP_NO_OP / EPP + ((_NOP_NO_OP % EPP) ? 1 : 0))
    // * GRAIN_PER_PAGE * 4 / 3;	// 0.75 is for hash table load factor
    env->nr_valid_tpages =
        (_NOP_NO_OP / EPP + ((_NOP_NO_OP % EPP) ? 1 : 0)) * GRAIN_PER_PAGE;
    env->nr_valid_tentries = env->nr_valid_tpages * EPP;
    // env->max_cache_entry = (_NOP_NO_OP / EPP + ((_NOP_NO_OP % EPP) ? 1 : 0))
    // * GRAIN_PER_PAGE * 4 / 3; 	// number of tpages
    env->max_cache_entry =
        (_NOP_NO_OP / EPP + ((_NOP_NO_OP % EPP) ? 1 : 0)) * GRAIN_PER_PAGE;
...
static void cache_member_init(struct cache_member *member) {
    struct cmt_struct **cmt =
        g_malloc0(d_cache.env.nr_valid_tpages * sizeof(struct cmt_struct *));
    ...
    member->mem_table =
        g_malloc0(d_cache.env.nr_valid_tpages * sizeof(struct pt_struct *));
    for (int i = 0; i < d_cache.env.nr_valid_tpages; i++) {
#ifdef CMT_USE_NUMA
        member->mem_table[i] =
            numa_alloc_onnode(EPP * sizeof(struct pt_struct), 1);
...
int dftl_cache_load(demand_cache *self, lpa_t lpa, request *const req,
                    snode *wb_entry) {
...
int dftl_cache_list_up(demand_cache *self, lpa_t lpa, request *const req,
                       snode *wb_entry) {
    int rc = 0;
    ...
    if (self->is_full(self)) {
        ...
        if (victim->state == DIRTY) {
            ...
            bm_oob_t new_oob = {
                .is_tpage = true, .lpa = victim->idx, .length = PAGESIZE};
    ...
    if (cmt->is_flying) {
        ...
        if (req) {
            ...
            while (ring_dequeue(cmt->retry_q, (void *)&retry_wbe, 1)) {
                // lpa_t retry_lpa = get_lpa(retry_wbe->key,
                // retry_wbe->hash_params);

                struct inflight_params *i_params = get_iparams(NULL, retry_wbe);
                i_params->jump = GOTO_COMPLETE;
                // i_params->pte = cmt->pt[OFFSET(retry_lpa)];
                algo_q_insert_sorted(pw_buffer->env->wb_retry_q, NULL,
                                     retry_wbe);
            }
        }
    }
...
int dftl_cache_wait_if_flying(demand_cache *self, lpa_t lpa, request *const req,
                              snode *wb_entry) {
    struct cmt_struct *cmt = self->member.cmt[D_IDX];
    if (cmt->is_flying) {
