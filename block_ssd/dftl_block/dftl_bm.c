...
#ifdef DATA_SEGREGATION
  ftl_assert(env->part[1].free_sblk_cnt > MAX_GC_STREAM);
  for (int i = env->part[1].s_sblk; i < env->part[1].s_sblk + MAX_GC_STREAM;
       i++) {
    env->sblk[i].is_rsv = true;
  }
  env->part[1].free_sblk_cnt -= MAX_GC_STREAM;
  env->stream = (bm_stream_manager_t *)g_malloc0(sizeof(bm_stream_manager_t) *
                                                 MAX_GC_STREAM);
  for (int i = 0; i < MAX_GC_STREAM; i++) {
    env->stream[i].idx = i;
    env->stream[i].active_sblk =
        self->get_active_superblock(pbm, DATA_S, FALSE);
    env->stream[i].active_sblk->stream_idx = i;
    env->stream[i].active_sblk->is_flying = true;
    env->stream[i].active_sblk->is_rsv = false;
    env->stream[i].active_ppa =
        self->get_page_num(self, env->stream[i].active_sblk);
    env->stream[i].flush_ppa = env->stream[i].active_ppa;
    int page_remain = SBLK_OFFT2PPA(env->stream[i].active_sblk, SBLK_END) -
                      env->stream[i].active_ppa;
...
bm_superblock_t *dftl_get_stream_sblk(block_mgr_t *self, lpa_t stream_idx,
  ...
  if (self->check_full(self, stream->active_sblk)) {
    ...
    if (unlikely(stream->active_sblk == NULL)) {
      ftl_err("no active superblock for stream=%d (is_reserve=%d)\n", idx,
              (int)is_reserve);
      abort();
    }
    ...
    if (is_reserve) {
      ...
      // full_sblk->index, stream->active_sblk->index);
...
void dftl_show_sblk_state(block_mgr_t *self, int pt_num) {
  ...
  ftl_log("------------sblk state-------------\n");
  for (int i = pt->s_sblk; i <= pt->e_sblk; i++) {
    ftl_log("sblk[%d]: stream=%d, start_ppa=%llu, is_rsv=%d, is_full=%d, "
            "valid_cnt=%u, wp=%d, is_flying=%s\n",
            i, env->sblk[i].stream_idx,
            (unsigned long long)(env->sblk[i].index * _PPS),
            (env->sblk[i].is_rsv == true),
            self->check_full(self, &env->sblk[i]), env->sblk[i].valid_cnt,
            env->sblk[i].wp_offt, env->sblk[i].is_flying ? "true" : "false");
  }
  ftl_log("------------stream state-------------\n");
  for (int i = 0; i < MAX_GC_STREAM; i++) {
    bm_env_t *env = self->env;
    bm_stream_manager_t *stream = &env->stream[i];
    ftl_log("stream[%d] (sblk[%d]): page_remain=%d, grain_remain=%d, "
            "active_ppa=%d, flush_ppa=%d, flush_page=%d, sblk_wp=%d\n",
            stream->idx, stream->active_sblk->index, stream->page_remain,
            stream->grain_remain, stream->active_ppa, stream->flush_ppa,
            stream->flush_page, stream->active_sblk->wp_offt);
  }
  ...
  if (pt_num == DATA_S) {
    ...
    for (int i = pt->s_sblk; i <= pt->e_sblk; i++) {
      if (env->sblk[i].is_flying == true)
        continue;
      if (env->sblk[i].is_rsv == false &&
          env->sblk[i].stream_idx == stream_idx &&
          self->check_full(self, &env->sblk[i]) &&
    ...
    /* Fallback: if the current stream has no full victim, pick global full
     * victim. */
    if (target == NULL) {
      for (int i = pt->s_sblk; i <= pt->e_sblk; i++) {
        if (env->sblk[i].is_flying == true)
