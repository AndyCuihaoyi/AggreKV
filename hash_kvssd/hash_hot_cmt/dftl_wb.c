...
#ifdef DATA_SEGREGATION
    /* DS 模式：per-stream flush list（size = MAX_WRITE_BUF，和 wb 1:1）。
     * 流分派在 _do_wb_assign_ppa 时按 D_IDX%MAX_GC_STREAM 决定写到哪个流。 */
    for (int i = 0; i < MAX_GC_STREAM; i++) {
        env->fl_stream[i] = (struct flush_list *)malloc(sizeof(struct flush_list));
        env->fl_stream[i]->size = 0;
        env->fl_stream[i]->list = (struct flush_node *)calloc(
...
#ifdef DATA_SEGREGATION
    for (int i = 0; i < MAX_GC_STREAM; i++) {
        free(env->fl_stream[i]->list);
        free(env->fl_stream[i]);
        env->fl_stream[i] = NULL;
    }
#else
...
// #define FOR_WB_WR_HIT_STATS
    ...
    /* Ownership is transferred to wb_entry and released after flush. */
    req->h_params = NULL;
...
void _do_wb_assign_ppa(w_buffer_t *self, request *req)
{
...
#ifdef DATA_SEGREGATION
    /* DS 模式：per-stream flush list。
     *  每个 wb_entry 按 D_IDX%MAX_GC_STREAM 决定落到哪个流的 fl_stream[idx]。
     *  流内连续段判断照搬单流（ppa != last.ppa+last.length 时开新段）。
     *  wb 满时所有流统一 flush。 */
    ftl_assert(env->fl_stream[0] != NULL);
    for (size_t i = 0; i < MAX_WRITE_BUF; i++)
    {
        ...
        struct flush_list *fl = env->fl_stream[stream_idx];

        ppa_t ppa;
        if (stream->grain_remain == 0)
        {
            ppa = dp_alloc(pbm, lpa);
            /* 同步 active_sblk_offset = active_sblk->wp_offt */
            stream->active_sblk_offset = stream->active_sblk->wp_offt;
            stream->active_ppa = ppa;
            stream->grain_remain = GRAIN_PER_PAGE;
            /* 连续段判断（与单流完全一致），但只在该流的 fl_stream[idx] 内部 */
            if (fl->size == 0 || ppa != fl->list[fl->size - 1].ppa + fl->list[fl->size - 1].length)
            {
                fl->list[fl->size].ppa = ppa;
                fl->list[fl->size].length = 1;
                fl->list[fl->size].value = NULL;
                fl->list[fl->size].value_offt = 0;
                fl->size++;
            }
            else
            {
                fl->list[fl->size - 1].length++;
            }
            stream->flush_ppa = ppa;
            stream->flush_page = 1;
        }
        else
        {
...
void _do_wb_mapping_update(w_buffer_t *self, request *req)
{
    ...
    wb_retry:
        ...
        if (unlikely(h_params->cnt > MAX_HASH_COLLISION))
        {
            ftl_err("hash collision overflow in wb path: cnt=%d, key_len=%u, key=%.*s, hash=%u\n",
                    h_params->cnt, wb_entry->key.len, wb_entry->key.len,
                    wb_entry->key.key, h_params->hash);
            abort();
        }
        ...
        if (IS_INFLIGHT(wb_entry->params))
        {
            ...
            case GOTO_UPDATE:
                goto wb_update;
            default:
                ftl_err("No jump type found\n");
                abort();
            }
        }
#ifdef HOT_CMT
        bool hot_hit = false;
        bool hot_hit_key_mismatch = false;
        KEYT hot_key;
        if (h_params->cnt == 0 && pd_cache->hot_is_hit(pd_cache, lpa, &hot_pte) == true)
        {
            ...
            {
                hot_hit = true;
                dftl_cache_cold_grain_access(pd_cache, lpa);
            }
#ifdef ADAPTIVE_MEM
            else if (hot_pte->key_ghost_mark)
            {
                hot_hit_key_mismatch = true;
            }
#endif
        }
        if (hot_hit_key_mismatch)
        {
#ifdef ADAPTIVE_MEM
            /* Update 时 key_fp 不匹配（old ≠ new），走 adaptive_ghost_key_on_flash_verify
             * 路径：若 key_ghost_mark 且 key_fp 匹配则计数；不匹配则返回 false 不计数。
             * 为确保 Update 路径的 ghost 命中被计数，当 hot hit + key mismatch
             * + real_key 已清空（说明 key 缓存曾被驱逐，等效于 ghost），直接计数。 */
            bool counted = adaptive_ghost_key_on_flash_verify(pd_cache, hot_pte,
								  h_params->key_fp);
            (void)counted;
#endif
        }
        if (h_params->cnt == 0 && pd_cache->hot_is_hit(pd_cache, lpa, &hot_pte) == true &&
            h_params->key_fp != hot_pte->key_fp
#ifdef VERIFY_CACHE
            && hot_pte->real_key.len == 0
#endif
        )
        {
#ifdef ADAPTIVE_MEM
            hot_cmt_table_lock();
            pd_cache->stat.ghost_hit_key++;
...
#ifdef ADAPTIVE_MEM
            adaptive_ghost_cold_on_miss(pd_cache, pd_cache->member.cmt[D_IDX], true);
#endif
        wb_cache_load:
            // rc = pd_cache->wait_if_flying(pd_cache, lpa, NULL, wb_entry);
            //  if (rc)
            //      continue; /* pending */
    ...
    wb_data_check:
        ...
        int read_skip = false;
#ifdef VERIFY_CACHE
        if (hot_hit && hot_pte->real_key.len != 0)
        {
            if (KEYCMP(wb_entry->key, hot_pte->real_key) == 0)
            {
                read_skip = true;
...
#ifdef ADAPTIVE_MEM
            if (hot_hit)
                adaptive_ghost_key_on_flash_verify(pd_cache, hot_pte, h_params->key_fp);
#endif
            // read_for_data_check(pbm, pte.ppa, wb_entry);
            if (hot_hit && hot_pte->real_key.len == 0 &&
#ifdef ADAPTIVE_MEM
                !hot_pte_key_is_ghost(hot_pte) &&
#endif
                real_key->len <= MAX_CACHED_KEY_LEN &&
                pd_cache->env.max_cached_full_key >= real_key->len)
            {
                hot_pte->real_key = *real_key;
#ifdef ADAPTIVE_MEM
                hot_pte->key_ghost_mark = false;
#endif
    ...
    {
        ftl_err("wb_entry still remains in queues\n");
        abort();
    }
...
void _do_wb_flush(w_buffer_t *self, request *req)
{
    ...
    skiplist **wb = &self->wb;
#ifdef DATA_SEGREGATION
    /* DS 模式：遍历所有流的 fl_stream[idx]，每个流内按段 flush。
     * 至少有一个流有内容（assert）。 */
    bool any_dirty = false;
    for (int s = 0; s < MAX_GC_STREAM; s++) {
        if (env->fl_stream[s]->size > 0) {
            any_dirty = true;
            break;
        }
    }
    ftl_assert(any_dirty);
    uint64_t maxlat = 0;
    uint32_t tt_pgs = 0;
    for (int s = 0; s < MAX_GC_STREAM; s++)
    {
        flush_list *fl = env->fl_stream[s];
        for (int i = 0; i < fl->size; i++)
        {
            ...
            if (lat == (uint64_t)-1 || lat == (uint64_t)-2) {
                ftl_err("[wb flush stream%u] ppa=%llu length=%u lat=%lu\n",
                        s, (unsigned long long)ppa, fl->list[i].length, (unsigned long)lat);
                abort();
            }
    ...
    for (int i = 0; i < fl->size; i++)
    {
        ...
        uint64_t lat = __demand.li->write(ppa, (uint64_t)fl->list[i].length * PAGESIZE, 0);
        if (lat == (uint64_t)-1 || lat == (uint64_t)-2) {
            ftl_err("[wb flush] ppa=%llu length=%u lat=%lu\n",
                    (unsigned long long)ppa, fl->list[i].length, (unsigned long)lat);
            abort();
        }
        maxlat = lat > maxlat ? lat : maxlat;
        tt_pgs += fl->list[i].length;
