...
#include "dftl_wb.h"
#include "../include/x86_64-linux-gnu/sys/prctl.h"
...
void *end_request(request *req)
{
    ...
    case DATAR:
        ...
        else
        {
            ...
            if ((++finished_r_local) % 200000 == 0)
            {
                uint64_t now = clock_get_ns();
                double elapsed = (now - rd_start_ns) / 1e9;
                rd_start_ns = now;
                ftl_log("%lu th read request end. key: %s, iops: %lf, avg_lat: "
                        "%ld, slow: %ld\n",
                        finished_r_local, req->key.key,
                        (double)200000 / elapsed, sum_rlat_ns_local / 200000,
                        complete_r_slow);
    ...
    case DATAW:
    {
        ...
        uint64_t sum_wlat_ns_local =
            g_atomic_pointer_add(&sum_wlat_ns, (req->etime - req->stime));
        if ((++finished_w_local) % 200000 == 0)
        {
            ...
            wr_start_ns = now;
            ftl_log("%lu th write request end, iops: %lf, avg_lat: %ld, slow: "
                    "%ld\n",
                    finished_w_local, (double)200000 / elapsed,
                    sum_wlat_ns_local / 200000, complete_w_slow);
...
void *test_update_tr(void *args)
{
    ...
    for (uint64_t i = 0; i < num; ++i)
    {
        ...
        if (is_zipf)
            ...
            rndkey = rand() % (max - 1) + 1;
        // int len = rand() % 3500 + 9;    // variable, must larger than
        // len(str(rnd)) int len = 512;    // fixed, must larger than
        // len(str(rnd))
        char value_str[len];
        memset(value_str, 0, len);
        // sprintf(value_str, "%0*d", len - 1, rnd);
...
void show_stats()
{
    ...
    ftl_log("w_buffer: rd_hit: %lu, rd_miss: %lu, wr_hit: %lu, wr_miss: %lu\n",
            write_buffer.stats->nr_rd_hit, write_buffer.stats->nr_rd_miss,
            write_buffer.stats->nr_wr_hit, write_buffer.stats->nr_wr_miss);
    ...
    for (int i = 0; i < 64; ++i)
    {
        ftl_log("lun[%d]: rd: %ld, wr: %ld, er: %ld\n", i,
                ssd_li.stats->nr_nand_rd_lun[i],
                ssd_li.stats->nr_nand_wr_lun[i],
...
int main(int argc, char **argv)
{
    ...
    toggle_ssd_lat(true);
    test_read(palgo, pool_size, num_read, false, seed, 8);
    ftl_log("finish random reading.\n");
    fflush(stdout);
    sleep(5);
    // ftl_log("start zipfian reading. iodepth: %d\n", iodepth);
    // test_read(palgo, pool_size, num_read, true, seed, 8);
