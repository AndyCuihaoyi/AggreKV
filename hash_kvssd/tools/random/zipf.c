							 ...
							 double center, unsigned int seed)
{
	...
	} else if (!zs->disable_hash) {
		/* shuffle_map 模式下，所有 worker 必须用相同的 rand_off=0
		 * 才能保证跨 worker 热点一致；disable_hash=true（无 shuffle_map）
		 * 时保持原有随机偏移行为。 */
		zs->rand_off = 0;
	}
}
...
void zipf_use_shuffle_map(struct zipf_state *dst_zs, uint64_t *shuffle_map)
{
	...
	/* shuffle_map 模式下强制 rand_off=0，保证跨 worker 热点一致；
	 * disable_hash 已由 zipf_disable_hash() 设为 true，但 shuffle_map
	 * 本身已能打乱分布，rand_off=0 不影响正确性。 */
	dst_zs->rand_off = 0;
}
