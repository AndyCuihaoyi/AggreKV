...
int iodepth = 16;
pthread_spinlock_t nr_ios_lock;
...
void *end_request(request *req) {
    ...
    case DATAR:
        if (req->state == ALGO_REQ_NOT_FOUND) {
            ...
            sum_rlat_ns += (req->etime - req->stime);
            if (finished_r % 200000 == 0) {
                uint64_t now = clock_get_ns();
                double elapsed = (now - rd_start_ns) / 1e9;
                rd_start_ns = now;
                ftl_log("%lu th read request end. key: %s, iops: %lf, avg_lat: %ld, slow: %ld\n", finished_r, req->key.key, (double)200000 / elapsed, sum_rlat_ns / 200000, complete_r_slow);
                sum_rlat_ns = 0;
                complete_r_slow = 0;
                fflush(stdout);
                fflush(stderr);
    ...
    pthread_spin_lock(&nr_ios_lock);
    (*req->ptr_nr_ios)--;
    pthread_spin_unlock(&nr_ios_lock);
    free(req);
    return NULL;
}
...
void *algo_thread() {
    ...
    while (1) {
        request *req = NULL;
        uint64_t now = clock_get_ns();
        if (palgo->retry_q->head != NULL && ((request *)palgo->retry_q->head)->etime <= now) {
...
void show_stats() {
    ...
    ftl_log("w_buffer: rd_hit: %lu, rd_miss: %lu, wr_hit: %lu, wr_miss: %lu\n",
        write_buffer.stats->nr_rd_hit, write_buffer.stats->nr_rd_miss, write_buffer.stats->nr_wr_hit, write_buffer.stats->nr_wr_miss);
...
int main(int argc, char **argv) {
    ...
    ftl_log("pool_size = %lu, num_update = %lu, num_read = %lu, map_size_frac = %lu, seed = %d\n", pool_size, num_update, num_read, map_size_frac, seed);
    timer_start_ns = clock_get_ns();
    algorithm *palgo = &__demand;
    ssd_li.create(&ssd_li);
    palgo->create(palgo, &ssd_li);
    init_global_timer();
    pthread_spin_init(&nr_ios_lock, PTHREAD_PROCESS_PRIVATE);
    if (map_size_frac > 1) {
    ...
    pthread_create(&finish_tr, &cq_cpl_attr, process_cq_cpl, NULL);
    ftl_log("start test.\n");
    iodepth = 64;
    ...
    sleep(1);
    iodepth = 64;
    ftl_log("start read warming. iodepth: %d\n", iodepth);
    toggle_ssd_lat(true);
    test_read(palgo, pool_size, 5000000, false, seed, 8);
    fflush(stdout);
    test_read(palgo, pool_size, 5000000, true, seed, 8);
    fflush(stdout);
    sleep(5);
    return 0;
    ...
    test_read(palgo, pool_size, num_read, true, seed, 1);
    ftl_log("read finished.\n");
    sleep(5);
    show_stats();
    clean_stats();
    iodepth = 64;
    ftl_log("start uniform reading. iodepth: %d\n", iodepth);
    test_read(palgo, pool_size, num_read, false, seed, 1);
    ftl_log("read finished.\n");
    sleep(5);
    ftl_log("test finished.\n");
    show_stats();
    clean_stats();
    // iodepth = 1;
    // ftl_log("start zipfian reading. iodepth: %d\n", iodepth);
    ...
    // clean_stats();
    for (int i = 0; i < INNER_TIMER_SIZE; ++i) {
        ftl_log("global_inner_timer[%d]: %lu\n", i, global_inner_timer[i].elapsed);
    }
    // while(1);
    return 0;
}
...
//         {"map_size_frac", required_argument, NULL, 3},
//         {"seed", required_argument, NULL, 4},
//         {"warm_update", required_argument, NULL, 5},
//         {0, 0, 0, 0},
//     };
//     timer_start_ns = clock_get_ns();
//     ftl_log("hello world\n");
...
//     uint64_t num_read = 0;
//     uint64_t map_size_frac = 1;
//     uint64_t num_warm_update = 1000000;
//     int seed = 1;
//     char *shortopts = "";
...
//             case 5:
//             // warm_update
//                 num_warm_update = atoi(optarg);
//                 break;
//             case '?':
//                 break;
//         }
//     }
//     if (!pool_size || !num_update) {
//         ftl_err("need pool_size and num_update\n");
//         abort();
//     }
//     ftl_log("pool_size = %lu, num_update = %lu, warm_update = %lu, num_read = %lu, map_size_frac = %lu, seed = %d\n", pool_size, num_update, num_warm_update, num_read, map_size_frac, seed);

//     algorithm *palgo = &__demand;
//     ssd_li.create(&ssd_li);
//     palgo->create(&ssd_li, palgo);
//     init_global_timer();
//     pthread_spin_init(&nr_ios_lock, PTHREAD_PROCESS_PRIVATE);
//     if (map_size_frac > 1) {
//         d_cache.env.nr_valid_tpages /= map_size_frac;
...
//     clean_stats();
//     sleep(5);
//     ftl_log("start updating warming.\n");
//     test_update(palgo, pool_size, num_warm_update, true, seed, 1);
//     sleep(5);
//     ftl_log("update warming finished.\n");
//     show_stats();
//     clean_stats();
//     sleep(5);
//     toggle_ssd_lat(true);
//     ftl_log("start updating.\n");
