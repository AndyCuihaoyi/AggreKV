...
int hash_collision_logging(int cnt, rw_t type) {
	if (cnt < 0 || cnt > MAX_HASH_COLLISION) {
		return 1;
	}
...
int ftl_try_mlock(const void *addr, size_t len, const char *tag)
{
	static int warned;
	...
	if (!warned)
	{
		ftl_warn("mlock(%s, %zu B) failed errno=%d (%s); continuing without "
			 "pinned memory (ulimit -l / /etc/security/limits.conf memlock)\n",
			 tag ? tag : "region", len, errno, strerror(errno));
		warned = 1;
	}
	return -1;
}
