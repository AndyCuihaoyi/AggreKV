...
- All sub-drivers write to per-experiment working directories under each module's root (e.g., `hash_kvssd/hash_kvssd_results/`, `lsmtree_kvssd/lsmtree_test_results/`, `block_ssd/blktrace_test_results/`). These are `.gitignore`d and regenerated by each sub-driver. The `hash_kvssd` sub-driver emits only `.log` files under Functional-badge review; no aggregate summary file is written.
