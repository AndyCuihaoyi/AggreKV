...
#define HOT_CMT_FAC (0.1)
#define FULL_KEY_FAC (0.10)
#define MAX_PROBE (30)
#define HOT_CMT_PROMOTE_THRESHOLD (70)
#endif
...
#ifdef HOT_CMT
	env->max_cached_tpages = ceil(_NOP_NO_OP / 1024 * (1 - HOT_CMT_FAC - FULL_KEY_FAC));
	env->max_cached_hot_tpages = ceil(_NOP_NO_OP / 1024 * HOT_CMT_FAC);
	env->max_cached_full_key = ceil(_NOP_NO_OP / 1024 * HOT_CMT_FAC) * (PAGESIZE / MAXKEYSIZE);
...
int dftl_cache_promote_hot(demand_cache *self, lpa_t lpa, request *const req, snode *wb_entry, struct cmt_struct *victim)
{
	...
	for (int i = 0; i < EPP; i++)
	{
		...
		{
			total_valid_entry++;
			if (victim->cnt_map[i] >= STATE_NUM - 1)
			{
				hot_entry_cnt++;
			}
		}
	...
	// uint32_t hot_entry_ratio = (hot_entry_cnt * 100) / total_valid_entry;
	// if (hot_entry_ratio >= HOT_ENTRY_RATIO_THRESH)
	// {
	// 	return 0;
	// }
	for (int i = 0; i < EPP; i++)
	{
		...
		if (victim->pt[i].ppa != UINT32_MAX && victim->cnt_map[i] >= T)
		{
			...
			{
				self->member.hot_mem_table[d_idx][p_idx].state -= 1;
				// linear
				// p_idx = (p_idx + 1);
				// quadratic
			...
			if (found == true || (found == false && DEFAULT_STATE > self->member.hot_mem_table[d_idx][p_idx].state))
			{
				...
				self->member.hot_mem_table[d_idx][p_idx].state = DEFAULT_STATE;
#ifdef VERIFY_CACHE
				if (self->member.hot_mem_table[d_idx][p_idx].real_key.len != 0)
				{
					pd_cache->env.max_cached_full_key++;
				}
#endif
			}
#ifdef VERIFY_CACHE
...
int dftl_cache_list_up(demand_cache *self, lpa_t lpa, request *const req, snode *wb_entry)
{
	...
	if (self->is_full(self))
	{
		...
		pop_count++;
		if (pop_count % Promote_THRESHOLD == 0)
		{
			victim = (struct cmt_struct *)lru_pop_hot(self->member.lru);
			if_promote = true;
		}
		else
		{
